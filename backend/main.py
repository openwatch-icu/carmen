...
def search_cameras():
    """
    Geocode the given location and query Shodan for nearby cameras.
    Returns camera metadata only — no credentials are tested or bypassed.
    """
    ...
    # Optional: caller may supply up to 5 custom base query strings.
    # The geo: filter is still appended server-side.
    ...
    coords = asyncio.run(geocode_location(location))
    if coords is None:
        return jsonify(
        ...
        if status == 403:
            return jsonify(
                error=(
                    ...
                    "The geo: radius filter is only available on paid "
                    "Shodan plans. Upgrade at shodan.io."
                ),
...
def proxy_image():
    """
    Proxy a still image from a camera, trying multiple snapshot paths.
    Only fetches images that respond without credentials.
    ...
    # Build ordered list of paths to try: brand-specific first, then generic
    paths = SNAPSHOT_PATHS.get(brand, []) + SNAPSHOT_PATHS["_generic"]

    async def _try_paths() -> httpx.Response | None:
        async with httpx.AsyncClient(
            ...
            for path in paths:
                ...
                if resp.status_code in (401, 403, 407):
                    # Auth required — no point trying other paths
                    return resp
...
def stream_start():
    ...
    if stream_id is None:
        return jsonify(
            ...
            "Camera may be offline, auth-protected, or unreachable."
        ), 502
...
def _reject_private_ip(ip: str) -> None:
    ...
    try:
        ...
        # Reject hostnames — DNS-rebinding can make them resolve to
        # private IPs at request time, bypassing the check below.
        raise ValueError("Only IP addresses are accepted.")

    # Unwrap IPv4-mapped IPv6 (e.g. ::ffff:127.0.0.1) so the
    # private/loopback checks below apply to the inner IPv4 address.
